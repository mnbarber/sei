...
|---|---|---|
| `find` | Returns an array of all documents matching the _query object_ | `const movieDocs = await Movie.find({mpaaRating: 'PG'})`|
|`findById` | Find a document based on it's `_id` | `const movieDoc = await Movie.findById(req.params.id)`|
|`findOne`| Find the first document that matches the _query object_ | `const movieDoc = await Movie.findOne({releaseYear: 2000})`|
