...
# NOTE IF YOUR BRANCH IS MASTER INSTEAD OF MAIN, USE MASTER WHEREVER YOU SEE MAIN
...
Next, make sure that all changes to your code are committed to main, or (master)  in your local repository.

Although not necessary, go ahead and push the latest commit to _origin_:<br>`git push origin main` or `master`, check your branch using `git branch` in the terminal
...
- Merge the commits from `master`: `git merge main`
